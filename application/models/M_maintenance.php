...
class M_maintenance extends CI_Model
{
    ...
    {
       $alternatif = $this->db->get_where('alternatif', ['bidang' => 'maintenance'])->result_array();
        $dataHasil = [];
        $data = [];
        for ($i = 0; $i < count($alternatif); $i++) {
            ...
            $data[$i]['c14'] = (int)$alternatif[$i]['c14'];
        }
        $tc1 = 0;
        $tc2 = 0;
        $tc3 = 0;
        ...
        foreach ($data as $i => $dt) {
            ...
            for ($i = 0; $i < 4; $i++) {
                ...
                $arr['bn14'] = $dt['c14'] / $dataHasil['tc14'];
            }
            $bn[] = $arr;
        }
        return $bn;
    ...
    public function normalisasi_terbobot()
    {
       $alternatif = $this->db->get_where('alternatif', ['bidang' => 'maintenance'])->result_array();
        ...
        $bobotproses = $this->db->get_where('kriteria', ['bidang' => 'maintenance'])->result_array();
        foreach ($data as $i => $dt) {
            $arr = [];
            $arr['kode_alternatif'] = $alternatif[$i]['kode_alternatif'];
            $arr['nama_alternatif'] = $alternatif[$i]['nama_alternatif'];
            ...
            for ($j = 0; $j < count($bn); $j++) {
                ...
                $arr['bnt14'] = $bobotproses[0]['c14'] * $bn[$i]['bn14'];
            }
            $bnt[] = $arr;
        }
        return $bnt;
    ...
    public function solusi_ideal_P()
    {
        ...
        // Nilai A+
        $aP = [$max1, $max2, $max3, $max4, $min12, $min14];
        return $aP;
    }
    ...
    public function solusi_ideal_M()
    {
        ...
        // Nilai A-
        $aM = [$min1, $min2, $min3, $min4, $max12, $max14];
        return $aM;
    }
    ...
    public function solusi_ideal()
    {
        ...
        for ($i = 0; $i < count($bnt); $i++) {
            ...
            $solusi_ideal[$i]['data_positif'] = sqrt(($bnt[$i]['bnt1'] - $max1) ** 2 + ($bnt[$i]['bnt2'] - $max2) ** 2 + ($bnt[$i]['bnt3'] - $max3) ** 2 + ($bnt[$i]['bnt4'] - $max4) ** 2 + ($bnt[$i]['bnt12'] - $min12) ** 2 + ($bnt[$i]['bnt14'] - $min14) ** 2);
            $solusi_ideal[$i]['data_negatif'] = sqrt(($bnt[$i]['bnt1'] - $min1) ** 2 + ($bnt[$i]['bnt2'] - $min2) ** 2 + ($bnt[$i]['bnt3'] - $min3) ** 2 + ($bnt[$i]['bnt4'] - $min4) ** 2 + ($bnt[$i]['bnt12'] - $max12) ** 2 + ($bnt[$i]['bnt14'] - $max14) ** 2);
        }
        return $solusi_ideal;
        // return $maxc1;
    ...
    public function preferensi()
    {
        ...
        for ($i = 0; $i < count($solusi_ideal); $i++) {
            ...
            $preferensi[$i]['bidang'] = 'Maintenance';
