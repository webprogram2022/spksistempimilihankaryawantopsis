...
class M_limbah extends CI_Model
{
    ...
    {
       $alternatif = $this->db->get_where('alternatif', ['bidang' => 'limbah'])->result_array();
        $dataHasil = [];
        $data = [];
        for ($i = 0; $i < count($alternatif); $i++) {
            ...
            $data[$i]['c13'] = (int)$alternatif[$i]['c13'];
        }
        $tc1 = 0;
        $tc2 = 0;
        $tc3 = 0;
        ...
        foreach ($data as $i => $dt) {
            ...
            for ($i = 0; $i < 4; $i++) {
                ...
                $arr['bn13'] = $dt['c13'] / $dataHasil['tc13'];
            }
            $bn[] = $arr;
        }
        return $bn;
    ...
    public function normalisasi_terbobot()
    {
       $alternatif = $this->db->get_where('alternatif', ['bidang' => 'limbah'])->result_array();
        ...
        $bobotproses = $this->db->get_where('kriteria', ['bidang' => 'Limbah'])->result_array();
        foreach ($data as $i => $dt) {
            $arr = [];
            $arr['kode_alternatif'] = $alternatif[$i]['kode_alternatif'];
            $arr['nama_alternatif'] = $alternatif[$i]['nama_alternatif'];
            ...
            for ($j = 0; $j < count($bn); $j++) {
                ...
                $arr['bnt13'] = $bobotproses[0]['c13'] * $bn[$i]['bn13'];
            }
            $bnt[] = $arr;
        }
        return $bnt;
    ...
    public function solusi_ideal()
    {
        ...
        for ($i = 0; $i < count($bnt); $i++) {
            ...
            $solusi_ideal[$i]['data_positif'] = sqrt(($bnt[$i]['bnt1'] - $max1) ** 2 + ($bnt[$i]['bnt2'] - $max2) ** 2 + ($bnt[$i]['bnt3'] - $max3) ** 2 + ($bnt[$i]['bnt4'] - $max4) ** 2 + ($bnt[$i]['bnt13'] - $max13) ** 2);
            $solusi_ideal[$i]['data_negatif'] = sqrt(($bnt[$i]['bnt1'] - $min1) ** 2 + ($bnt[$i]['bnt2'] - $min2) ** 2 + ($bnt[$i]['bnt3'] - $min3) ** 2 + ($bnt[$i]['bnt4'] - $min4) ** 2 + ($bnt[$i]['bnt13'] - $min13) ** 2);
        }
        return $solusi_ideal;
        // return $maxc1;
